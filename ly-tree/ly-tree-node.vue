<template>
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{ 
		...
		<view class="ly-tree-node__content" @tap.stop="handleClick" :class="{
			...
			<!-- 使用data中的字段进行叶子节点验证 -->
			<ly-checkbox v-if="checkboxVisible || radioVisible && node.data.isLeaf"
	...
	export default {
		...
		data() {
			return {
				...
				nodeISselect: false, //节点是否被选中，处理多选数据时需要
			};
		},
		...
		methods: {
			...
			// 处理节点被选择的数据
			multiSelect() {
				if (this.nodeISselect === true) {
					return
				} else {
					console.log('tree node handle!')
					this.nodeISselect = true
					this.tree.receiveNode(this.node)
					// 阻止点击事件多次触发
					setTimeout(()=>{this.nodeISselect = false},200)
				}
			},
