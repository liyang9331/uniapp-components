...
<script>
	...
	export default {
		...
		methods: {
			/*
			 * @description 接收树节点被选中时传递的数据
			 * @method receiveNode
			 * @param {Array} data 节点集合
			*/
			receiveNode(data) {
				// 触发节点点击事件
				if (this.store.multiSelect === true) {
					// 去重复
					let flag = false
					let index = 0
					this.nodeSelectList.forEach((item, key) => {
						if (item.id == data.id) {
							flag = true
							index = key
						} else {
					...
					if (flag === true) {
						...
						this.nodeSelectList.push(data)
					}
				} else {
					this.nodeSelectList.push(data)
				}
				this.$emit('node-click', this.nodeSelectList)
			},
		...
		created() {
			...
			this.store = new TreeStore({
				multiSelect:this.multiSelect,//是否多选
				key: this.nodeKey,
				data: this.treeData,
				lazy: this.lazy,
				props: props,
				...
				expandOnCheckNode: this.expandOnCheckNode,//节点被选中时展开节点
